...
% Cox is marked as 0 if no Cox must be considered
DataDC{2,7} = 0;
DataDC{2,8} = 0;
...
if strcmpi(buttonConsiderRsCox, 'Yes')
    ...
    % Ask the person if he wants to 
    promptMessage = sprintf('Is it a file ?');

    buttonFileorCosntant = questdlg(promptMessage, '..', 'Yes', 'No', 'Yes');
    
    
    if strcmpi(buttonFileorCosntant, 'Yes')
        ...
        % if he doesn't cancels it
            ...
            promptMessage = sprintf('Do you want to consider Cox - It will use the same formulas as for the MOS ?');
         ...
         resistancestr = inputdlg('Resistance in SI','Resisntace in SI');

         resistance = str2num(resistancestr{1});

         while choice == 0 && isempty(resistance) == 1

            promptMessage = sprintf('You have failed to Select a Resistance, do you still want to use Rs ?');
        ...
        if isempty(resistance) == 0


            ConsiderRs = 1;
            SeriesResistance = cell(1,1);
            SeriesResistance{1,1} = resistance;
...
if ConsiderRs == 0 && ConsiderOx ==0
    ...
    for t = 1:size(FilePathNAME,1)
        ...
        DynamicConductance{t,1} = Conductance{t,1}./omega{t,1};
...
DataDC(2,8) = SeriesResistance(1,1);    
    ...
    for t = 1:size(FilePathNAME,1)
        ...
        % Since no Cox is considered to be there, only the Rs is in series
        
        DynamicConductance{t,1} = ConductanceC./omega{t,1};
...
DataDC(2,7) = OxydeCapacitance(1,1);    
    ...
    for t = 1:size(FilePathNAME,1)
        ...
        % To take into account the Series Resistance in the conductance
        GCnumerator = ((omega{t,1}.^2).*SeriesResistance{1,1}.*...
                            Capacitance{t,1}.*CapacitanceC) -...
                                Conductance{t,1};
        GCdenominator = (SeriesResistance{1,1}.*Conductance{t,1} - 1);
...
title([figname '-' 'Dynamic Conductance']);
