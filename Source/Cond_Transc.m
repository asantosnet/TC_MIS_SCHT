function [figureCTGP, DataCTPG, VectDataSheet, VectInfoSheet]...
...
%   the conductance/transconductance or normalized conductance/transcondu
%   ctance, gain and phase
...
DataCTPG = cell((Nsweeps+1),(size(Valeurs,2)+ Nparameters));
...
for t = 1:Nsweeps
    ...
    DataCTPG(1,(Nparameters+d)) = Valeurs(1,d);
...
for t = 1:size(Valeurs,2)
    ...
    DataCTPG{2,Nparameters+t} = Valeursvect;

end


% To help reading DataCTPG
VectDataSheet = [2 3 4 5 6 7 8];

VectInfoSheet = 9:size(DataCTPG,2);
...
for nplot = 1:size(FilePathNAME,1)
    ...
    handlesPhase(nplot) = plot(frequency{nplot,1},Phase{nplot,1},'-','Color',cmap(nplot,:));
...
for nplot = 1:size(FilePathNAME,1)
    ...
    handlesGain(nplot) = plot(frequency{nplot,1},Gain{nplot,1},'-','Color',cmap(nplot,:));
